...
def comment_notify(sender, instance, created, **kwargs): 
    ...
    if instance.approved_comment:
        subject = f'{instance.author}, Благодарим за Ваш отзыв, мы его приняли'
        body = f'Dear {instance.author}, Благодарим за Ваш отзыв от {instance.date_added.strftime("%d-%m-%Y")} по объявлению "{instance.post.title}" by {instance.post.author} принят ...'
        email = instance.author.email

    if not instance.approved_comment:
        subject = f'{instance.author}, Извините, Ваш отзыв отклонен...'
        body = f'Dear {instance.author}, Извините, Ваш отзыв от {instance.date_added.strftime("%d-%m-%Y")} по объявлению "{instance.post.title}" by {instance.post.author} отклонен ...'
        email = instance.author.email

    if created:
        subject = f'Dear {instance.post.author}, для Вас новый отзыв {instance.author} по объявлению "{instance.post.title}" ...'
        body = f'Dear {instance.post.author}, для Вас новый отзыв по объявлению "{instance.post.title}" от {instance.author} на {instance.date_added.strftime("%d-%m-%Y %H:%M")} ...'
        email = instance.post.author.email

    msg = EmailMultiAlternatives(
        subject=subject,
        body=body,
        from_email='3552@yandex.ru',
        to=[email]
    )
...
def delete_comment_notify(sender, instance, **kwargs): 
    ...
    subject = f'{instance.author}, к сожалению, Ваш отзыв удален...'
